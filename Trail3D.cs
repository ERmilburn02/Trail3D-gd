...
public partial class Trail3D : Node3D
{
	...
	public override void _Process(double delta)
	{
		...
		if (trailMesh == null)
		{
			GD.PushError("[TRAIL3D] TRAIL MESH COULD NOT BE CREATED.");
			emitting = false;
			return;
		}
		...
		if (snapshotTime > snapshotInterval)
		{
			...
			if (count > 0)
			{
				...
				if (totalTime - snapshotBuffer[0].Time > duration)
				{
	...
	{
		snapshotBuffer.Add(new TargetSnapshot(GlobalPosition, GlobalTransform.Basis, totalTime));
	}
	...
	private void DrawFace(ImmediateMesh mesh, int index)
	{
		...
		*/
		...
		Triangle tri1 = new Triangle(new[] { vert1, vert2, vert3 }, new[] { normal, normal, normal }, tri1UVs, totalTime);
		...
		Triangle tri2 = new Triangle(new[] { vert4, vert3, vert2 }, new[] { normal, normal, normal }, tri2UVs, totalTime);
